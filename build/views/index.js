...
function printTrustLinesTable(lines) {
  ...
  _.each(lines, function (line) {
    table.push([line.counterparty, line.currency, line.limit]);
  });
...
function printPaymentsTable(payments) {
  ...
  for (var i = 0; i < payments.length; i++) {
    ...
    table.push([payment.id, payment.to_address_id, payment.to_amount, payment.to_currency]);
  }
  logger.info(table.toString());
}
...
function printRippleAddressesTable(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    table.push([address.id, address.user_id, address.managed, address.type, address.tag || "", address.createdAt]);
  }
  logger.info(table.toString());
}
...
function printExternalAccountsTable(accounts) {
  ...
  for (var i = 0; i < accounts.length; i++) {
    ...
    table.push([account.id, account.user_id, account.name]);
  }
  logger.info(table.toString());
}
...
function printUsersTable(users) {
  ...
  for (var i = 0; i < users.length; i++) {
    var user = users[i];
    table.push([user.id, user.name, user.createdAt]);
  }
  logger.info(table.toString());
}
...
function printExternalTransactionsTable(transactions) {
  ...
  for (var i = 0; i < transactions.length; i++) {
    ...
    table.push([transaction.id, transaction.createdAt, transaction.amount, transaction.currency, transaction.external_account_id]);
  }
  logger.info(table.toString());
}
...
function printAccountBalance(balances) {
  ...
  for (var i = 0; i < balances.length; i++) {
    ...
    table.push([balance.currency, balance.value, balance.counterparty]);
  }
  logger.info(table.toString());
}
